...
class CliftonStrengthsNode(NodeAbstract[list[ChatMessage]]):
    ...
    async def _ensure_client(self):
        if self.client is None:
            self.client = await (ReactAgentBuilder(agent_name=PersonaConstructorWorkflowAllPromptEN[3]["name"]).
                                 with_system_prompt(PersonaConstructorWorkflowAllPromptEN[3]["content"]).
                                 build("1008611"))
...
async def main():
    bigfivenode = CliftonStrengthsNode()

...
class MBTINode(NodeAbstract[list[ChatMessage]]):
    ...
    async def _ensure_client(self):
        if self.client is None:
            self.client = await (ReactAgentBuilder(agent_name=PersonaConstructorWorkflowAllPromptEN[2]["name"]).
                                 with_system_prompt(PersonaConstructorWorkflowAllPromptEN[2]["content"]).
                                 build("1008611"))
